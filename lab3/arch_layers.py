...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    activations = out.mean(dim=(0))
    # draw_feature_pics(activations, 'activations')
    sorted_activations, indices = torch.sort(activations)
    # draw_feature_pics(sorted_activations, 'sorted_activations')

    weight_last_layer = self.res2.feature2[0].weight
    bias_last_layer = self.res2.feature2[0].bias
    weight_last_layer[indices[:K]] = 0
    bias_last_layer[indices[:K]] = 0
